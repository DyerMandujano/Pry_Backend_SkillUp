...
public class RespuestaEstudianteController {
    ...
    @PostMapping("/batch")
    public ResponseEntity<List<RespuestaEstudiante>> crearRespuestas(@RequestBody List<RespuestaEstudiante> respuestas) {
        try {
            List<RespuestaEstudiante> guardadas = new ArrayList<>();
            for (RespuestaEstudiante respuesta : respuestas) {
                guardadas.add(respuestaService.guardarRespuesta(respuesta));
            }
